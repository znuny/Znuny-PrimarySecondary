...
# --
# $origin: znuny - ded9c741c09fe1dbcdad0c65b293f4c8fd470a9b - Kernel/Modules/AgentTicketActionCommon.pm
# --
...
# special markers has been set along the file to easy spot the differences introduced by ZnunyPrimarySecondary package
package Kernel::Modules::AgentTicketPrimarySecondary;
# ---
...
# ---
    ...
    my $PrimarySecondaryAdvancedEnabled = $ConfigObject->Get('PrimarySecondary::AdvancedEnabled') || 0;
# ---
...
# Znuny-PrimarySecondary
# ---
    if ($PrimarySecondaryAdvancedEnabled) {
        my $Display = $Config->{PrimarySecondaryMandatory} ? 2 : 1;
        $Config->{DynamicField}->{$PrimarySecondaryDynamicField} = $Display;
    }
# ---
...
# ---
            if (
                !$PrimarySecondaryAdvancedEnabled
                && $DynamicFieldConfig->{Name} eq $PrimarySecondaryDynamicField
                )
            {
                next DYNAMICFIELD;
            }
...
# Znuny-PrimarySecondary
# ---
            if ( $DynamicFieldConfig->{Name} eq $PrimarySecondaryDynamicField ) {
               $PossibleValuesFilter = $Self->_GetPrimarySecondaryData(
                Ticket => \%Ticket,
                PrimarySecondaryDynamicField => $PrimarySecondaryDynamicField,
               );
            }
# ---
...
# ---
    ...
    my $PrimarySecondaryAdvancedEnabled = $Kernel::OM->Get('Kernel::Config')->Get('PrimarySecondary::AdvancedEnabled') || 0;

    if ($PrimarySecondaryAdvancedEnabled) {
        $Config->{DynamicField}->{$PrimarySecondaryDynamicField} = 1;
    }
# ---
...
sub _GetPrimarySecondaryData {
    ...
    my $PrimarySecondaryDynamicField = $Param{PrimarySecondaryDynamicField};

    my $FieldValue = $Ticket{ 'DynamicField_' . $PrimarySecondaryDynamicField } || '';
    ...
    if ( $FieldValue !~ m{^(Primary|Master)$}xms ) {
        $Data{Primary} = $LayoutObject->{LanguageObject}->Translate('New Primary Ticket');
    }

    if ( $UnsetPrimarySecondary && $FieldValue =~ m{^(Primary|Master)$}xms ) {
        $Data{UnsetPrimary} = $LayoutObject->{LanguageObject}->Translate('Unset Primary Ticket');
    }
    if ( $UnsetPrimarySecondary && $FieldValue =~ m{^(SecondaryOf|SlaveOf):(.*?)$}xms ) {
        $Data{UnsetSecondary}  = $LayoutObject->{LanguageObject}->Translate('Unset Secondary Ticket');
    }
    ...
    if ( $UpdatePrimarySecondary ) {
        ...
        for my $TicketID (@TicketIDs) {
            ...
            next TICKETID if $FieldValue eq "SecondaryOf:$CurrentTicket{TicketNumber}" || $FieldValue eq "SlaveOf:$CurrentTicket{TicketNumber}";
